...
module.exports = function generate(ctx) {
  ...
  Object.keys(ctx.models).forEach(modelName => {
    ...
    //console.log(ctx.models[modelName].relations);
      ...
      schema.push(
           ...
           */
          {
            ...
            params: {
              ...
              buildUIConf: buildUIConf,
              buildUIConfRel: buildUIConfRel,
              buildModelImports,
   ...
   */
  function buildModelProperties(model, isInterface) { // THIS
  ...
  function buildUIConf(property) {
    ...
    if (property && property.ui) {
      conf = Object.assign({}, confDefault, property.ui);
    } else
      conf = confDefault;
  ...
  function buildUIConfRel(model, relName) {

    let modelFile = process.cwd() + '/common/models/' + getModelFileName(model.name) + '.json';
    let c = require(modelFile);

    let rel = c.relations[relName];
    ...
    if (rel && rel.ui) {
      confDefault.type = rel.type;
      confDefault.model = rel.model;
      conf = Object.assign({}, confDefault, rel.ui);
    } else
      conf = confDefault;
